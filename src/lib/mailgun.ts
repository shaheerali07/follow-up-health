...
export function buildEmailHtml(opts: BuildEmailHtmlOptions): string {
  const { customContent, snapshotBlock, placeholders, ctaUrl } = opts;
  const resolvedCtaUrl = ctaUrl || process.env.NEXT_PUBLIC_APP_URL || '';

  console.log('[EMAIL] CTA URL:', resolvedCtaUrl || '(empty)');
  console.log('[EMAIL] NEXT_PUBLIC_APP_URL env:', process.env.NEXT_PUBLIC_APP_URL || '(not set)');
  if (!resolvedCtaUrl) {
    console.warn('[EMAIL] Warning: CTA URL is empty. NEXT_PUBLIC_APP_URL may not be set.');
  }
  ...
  const replacePlaceholders = (text: string) =>
    text
      ...
      .replace(/\{\{cta_url\}\}/g, resolvedCtaUrl);
